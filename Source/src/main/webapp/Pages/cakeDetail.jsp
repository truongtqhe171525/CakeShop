...
<head>
    ...
    <link rel="stylesheet" href="../Styles/cakeDetail.css"/>
    <link rel="stylesheet" href="../Styles/notificationSt.css"/>
...
<body>
    ...
    <header class="header">
        ...
        <nav class="header-navbar">
            <ul>
                ...
                <li class="header-target"><a href="/shop"> Shop </a></li>
    ...
    <main>
        ...
        <div class="cake-detail-container">
            <img class="cake-image" src="" alt="cake"/>
            <div class="card-content">
                <h1 class="cake-name"></h1>
                <h3 class="cake-description"></h3>
                <div class="detail-group">
                    <p> Kích thước: </p>
                    <ul class="list-cake-size"></ul>
                </div>
                <div class="detail-group">
                    <p> Giá: </p>
                    <div class="cake-old-price"></div>
                ...
                <div class="detail-group">
                    ...
                    <div class="quantity-input">
                        ...
                        <input id="quantity" type="number" name="quantity" min="1" max="1000" value="1"/>
                        <button onclick="handleQuantity(1)"> + </button>
                    </div>
                </div>

                <button class="btn-order" onclick="orderCake()"> Buy Now </button>
            </div>
        </div>
    </main>
    ...
    <div class="order-form-container" style="display: none;">
        <div class="form-outline">
            <button class="btn-close-order" onclick="document.getElementsByClassName('order-form-container')[0].style.display = 'none'"> <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="m289.94 256 95-95A24 24 0 0 0 351 127l-95 95-95-95a24 24 0 0 0-34 34l95 95-95 95a24 24 0 1 0 34 34l95-95 95 95a24 24 0 0 0 34-34z"></path></svg> </button>
            <form class="order-form" action="/cakeDetail" method="POST">
                <h1> More Infor </h1>
                <input type="hidden" name="idCake" value="${orderData.getId()}"/>
                <input type="hidden" name="sizeCake" value="${orderData.getSize()}"/>
                <input type="hidden" name="quantityCake" value="${orderData.getQuantity()}"/>
                <input type="hidden" name="total" value="${orderData.getTotal()}"/>
                <input type="hidden" name="idClient" value="${orderData.getIdClient()}"/>
                <input type="hidden" name="notice" value="${orderData.getNotice()}" id="noticeHidden"/>
                <div class="order-group">
                    <p> Notice: </p>
                    <textarea id="noticeShow" name="notice" placeholder="Notice" value="${orderData.getNotice()}"></textarea>
                    <i> ${failOrder.notice} </i>
                </div>
                <div class="order-group">
                    <p> Address: </p>
                    <input type="text" name="address" placeholder="Address" value="${orderData.getAddress()}"/>
                    <i> ${failOrder.address} </i>
                </div>
                <div class="order-group">
                    <p> Delivery Date: </p>
                    <input type="datetime-local" name="date" value="${orderData.getDate()}">
                    <i> ${failOrder.date} </i>
                </div>
                <div class="order-group">
                    <button> Order Now </button>
                    <i> ${failOrder.message} </i>
                </div>
            </form>
        </div>
    ...
    <script src="../Scripts/notificationSc.js"></script>
    <script src="../Scripts/loadUser.js"></script>
    <script src="../Scripts/handleStorage.js"></script>
    <script src="../Scripts/cakeDetail.js"></script>
    <script>
        loadUserSc(${userInfor})
        checkUser(${userInfor})
        loadDetail(${cakeDetail})
        loadQuantity(${orderData.getQuantity()})
        loadSize("${orderData.getSize()}")
        checkFail("${failOrder}")
        checkSaveOrder("${saveOrder}")
        document.getElementById('noticeShow').addEventListener('change', function () {
            document.getElementById('noticeHidden').value = this.value
        })
