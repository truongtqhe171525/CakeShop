...
<body>
    ...
    <script src="../Scripts/shopSc.js"></script>
    ...
    <script>
        ...
        let dataSlice = allCake.slice((indexCard - 1) * limitCard, indexCard * limitCard)
        loadCake(dataSlice)
        if(allCake == null || limitCard > allCake.length) document.getElementsByClassName("load-more")[0].style.display = 'none'

        const loadMore = () => {
            indexCard++;
            let dataSlice = allCake.slice((indexCard - 1) * limitCard, indexCard * limitCard)
            loadCake(dataSlice)
            if(indexCard * limitCard >= allCake.length){
